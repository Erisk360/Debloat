...
███████╗██║ ╚═╝ ██║██║ ╚═╝ ██║██║  ██║██║ ╚████║╚██████╔╝███████╗███████╗
╚══════╝╚═╝     ╚═╝╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝ ╚═════╝ ╚══════╝╚══════╝
                        Script by Emmanuel - Autopilot Debloat
"@
...
# Check if ExecutionPolicy has been bypassed; if not, relaunch script with Bypass
if ($env:ExecutionPolicyChecked -ne "1") {
    $env:ExecutionPolicyChecked = "1"
    Start-Process -FilePath "powershell.exe" -ArgumentList "-ExecutionPolicy Bypass -NoProfile -WindowStyle Hidden -File `"$PSCommandPath`"" -Wait
    exit
}

# Optional: Logging (recommended for troubleshooting)
$logPath = "C:\ProgramData\HP-DELL-LenovoDebloat.log"
"[$(Get-Date -Format u)] Script started with ExecutionPolicyChecked=$($env:ExecutionPolicyChecked)" | Out-File -Append $logPath
...
# Remove Netflix (typically installed as a UWP app)

# Remove UWP app - Netflix is usually distributed as a UWP app
Get-AppxPackage -AllUsers | Where-Object { $_.Name -like "*Netflix*" } | Remove-AppxPackage -AllUsers



# System-wide (Local Machine) - Requires admin
...
$DropboxApps = Get-ItemProperty `
    "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*", `
    "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*" `
    -ErrorAction SilentlyContinue | Where-Object {
        $_.DisplayName -like "*Dropbox-actie*"
    }

foreach ($app in $DropboxApps) {
    if ($app.UninstallString) {
        Write-Host "Uninstalling Dropbox: $($app.DisplayName)"
        $uninstallCmd = $app.UninstallString

        # Run the uninstaller silently
        if ($uninstallCmd -match "msiexec") {
            Start-Process "cmd.exe" -ArgumentList "/c $uninstallCmd /quiet /norestart" -Wait -WindowStyle Hidden
        } else {
...
Get-AppxPackage -AllUsers | Where-Object { $_.Name -like "*Dropbox*" } | Remove-AppxPackage -AllUsers
...
# Remove Dell SupportAssist (excluding version 3.2.x)
$SAVer = Get-ChildItem -Path HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall, `
                              HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall |
    Get-ItemProperty |
    Where-Object { $_.DisplayName -match "SupportAssist" } |
    Where-Object { $_.DisplayVersion -notlike "3.2*" } |
    Select-Object DisplayVersion, UninstallString, PSChildName
...
<Configuration>
  <Remove OfficeClientEdition="64">
    ...
    <Product ID="O365EducationRetail" />
    <Product ID="O365HomePremRetail" />
    <Product ID="O365PersonalRetail" />
    <Product ID="O365ProPlusVL" />
    <Product ID="O365Business" />
    <Product ID="O365Education" />
    ...
    <Product ID="HomeBusiness2016Retail" />
    <Product ID="HomeStudent2016Retail" />
    <Product ID="ProjectPro2019Retail" />
    <Product ID="ProjectStd2019Retail" />
    <Product ID="ProjectPro2016Retail" />
    <Product ID="ProjectStd2016Retail" />
    <Product ID="VisioPro2019Retail" />
    <Product ID="VisioStd2019Retail" />
    <Product ID="VisioPro2016Retail" />
    <Product ID="VisioStd2016Retail" />
  </Remove>
...
## Fallback DELl, Office, Onenote uninstall
...
Remove-M365 "Microsoft OneNote - it-it"
Remove-M365 "Microsoft OneNote - nl-nl"
Remove-M365 "Microsoft OneNote - en-us"
Remove-M365 "Microsoft OneNote - en-gb"
Remove-M365 "Microsoft OneNote - de-de"
Remove-M365 "Microsoft OneNote - fi-fi"  
